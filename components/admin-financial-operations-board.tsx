          ...
          <div className="max-h-[94vh] w-full overflow-y-auto rounded-t-[2rem] border border-white/60 bg-white/95 p-4 shadow-panel sm:max-h-[92vh] sm:max-w-5xl sm:rounded-[2rem] sm:p-6">
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0 flex-1">
                ...
                <h3 className="mt-2 text-xl font-semibold text-slate-900 sm:text-2xl">{selectedEntry.productName}</h3>
            ...
            <div className="mt-5 grid gap-5 xl:grid-cols-[0.92fr_1.08fr]">
              <div className="space-y-4">
                <div className="flex flex-col gap-4 rounded-[1.6rem] border border-slate-200 bg-slate-50/80 p-4 sm:flex-row sm:items-start">
                ...
                <div className="space-y-3 sm:hidden">
                  {selectedEntry.items.map((item) => (
                    <div key={`${item.id}-mobile`} className="rounded-[1.2rem] border border-slate-200 bg-white px-4 py-4">
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          <p className="text-sm font-semibold text-slate-900">{item.color}</p>
                          <p className="text-xs text-slate-500">Tamanho {item.size}</p>
                        </div>
                        <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">
                          {currency(item.confirmedTotalCost)}
                        </span>
                      </div>
                      <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-slate-600">
                        <div className="rounded-xl bg-slate-50 px-3 py-2">Solicitado: {item.requestedQuantity}</div>
                        <div className="rounded-xl bg-slate-50 px-3 py-2">Separado: {item.fulfilledQuantity}</div>
                      </div>
                    </div>
                  ))}
                </div>

                <div className="hidden overflow-hidden rounded-[1.4rem] border border-slate-200 bg-white sm:block">
              ...
              <div className="space-y-4">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <button
                    ...
                    className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-700"
                  >
