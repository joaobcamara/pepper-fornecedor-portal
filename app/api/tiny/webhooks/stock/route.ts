...
import { handleTinyStockWebhook, isTinyWebhookAuthorized } from "@/lib/tiny-stock-events";
...
function getTinyStockAccountKey(request: Request): TinyAccountKey {
...
export async function POST(request: Request) {
  ...
  try {
    const result = await handleTinyStockWebhook(payload, getTinyStockAccountKey(request));
    return NextResponse.json(result);
  } catch (error) {
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : "Falha ao processar webhook de estoque."
      },
      { status: 400 }
    );
  }
